...
function main() {
  ...
  if (!inputPath || !outputPath) {
    throw new Error("usage: node scripts/humanize-block-json.mjs <input-json-path> <output-json-path>");
  }

  const payload = JSON.parse(fs.readFileSync(path.resolve(inputPath), "utf8"));
  const block = payload.result;
  if (!block) {
    throw new Error("expected JSON-RPC response object with result field");
  }
