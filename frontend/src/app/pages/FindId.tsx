...
export default function FindId() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(payload.message ?? '가입된 계정이 있다면 아이디를 메일로 보냈습니다.');
    } catch (err) {
    ...
    <AppShell>
      <div className="mx-auto mt-6 w-full max-w-xl sm:mt-8">
        <section className="surface-panel p-6 fade-rise sm:p-8">
          <div className="mb-6">
            <div className="panel-title">아이디 찾기</div>
            <p className="panel-copy mt-2">가입 시 등록한 이메일을 입력하면 아이디를 메일로 보내드립니다.</p>
          </div>
